...
def overlay_classical_detection(
    ...
    ax.set_title(
        f"Classical: Gabor={gabor:.3f} FFT={fft:.3f} " f"Arc={arc_score:.3f} ({n_arcs} arcs)"
    )
...
def overlay_wavelet_detection(
    ...
    if spectrum:
        ...
        bar_colors = [scale_colors[min(i, len(scale_colors) - 1)] for i in range(len(spectrum))]
...
def overlay_variability(
    ...
    ax.set_title(f"Variability (score={score:.3f}, var={n_var}, per={n_per}, trans={n_trans})")
...
def overlay_anomaly_scores(
    ...
    bar_colors = ["#e74c3c" if s > 0.3 else "#f39c12" if s > 0.1 else "#95a5a6" for s in det_scores]
...
def _get_detector_scores(
    ...
    for name in detector_names:
        ...
        if len(keys) == 1:
            ...
            det_scores.append(float(detection.get(keys[0], {}).get(keys[1], 0)))
...
def create_annotated_summary(
    ...
    # Sources
    if len(positions) > 0:
        star_mask = np.array(sources.get("star_mask", [True] * len(positions)))
        ...
        if len(galaxies_pos) > 0:
            ax.scatter(
                ...
                edgecolors="lime",
                linewidth=0.5,
                label=f"Galaxies ({len(galaxies_pos)})",
            )
    # Overdensities
    ...
    ax.set_title(f"Lens={lens_score:.3f}{candidate_str} Sersic n={sersic_n:.1f}")
    ...
    if kinematic.get("no_catalog"):
        ...
        ax.set_title(
            f"Kinematic={kinematic.get('kinematic_score', 0):.3f} " f"(G={n_g} S={n_s} R={n_r})"
        )
    ...
    scored = [
        (name, det_scores[i]) for i, name in enumerate(detector_names) if det_scores[i] > 0.05
    ]
    scored.sort(key=lambda x: x[1], reverse=True)
    top_str = ", ".join(f"{n}={s:.3f}" for n, s in scored[:5]) if scored else "none above 0.05"
...
def create_evolution_summary(
    ...
    if all_single and any(generation_histories):
        ...
        if cycles_x:
            ax.plot(
                cycles_x, best_y, "o-", color="#e74c3c", linewidth=1.5, markersize=5, label="Best"
            )
            ax.plot(
                cycles_x,
                mean_y,
    ...
    for i, gen_hist in enumerate(generation_histories):
        ...
        if any(r is not None for r in rates):
            ...
            label = (
                f"Cycle {evolution_history[i].get('cycle', i) if i < len(evolution_history) else i}"
            )
    ...
    entries_with_components = [h for h in evolution_history if h.get("components")]
    if entries_with_components:
        x_labels = [str(h.get("cycle", i)) for i, h in enumerate(entries_with_components)]
        x_pos = np.arange(len(x_labels))
        bottom = np.zeros(len(x_labels))
        for j, comp_name in enumerate(component_names):
            values = [h["components"].get(comp_name, 0) for h in entries_with_components]
            ax.bar(
                x_pos,
                values,
                bottom=bottom,
